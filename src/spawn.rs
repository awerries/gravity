...
fn spawn_galaxy(
    ...
    for ii in 0..num_particles {
        ...
        let scale_r2 = (diameter * crate::GALAXY_SCALE_FACTOR / 2.0).powi(2);
...
fn get_star_color(mass: f32) -> Color
{
    ...
    let mass_ratio = (mass - crate::PARTICLE_MASS_LOWER) / (crate::PARTICLE_MASS_UPPER - crate::PARTICLE_MASS_LOWER);
...
fn random_orbital_circle_vel(r: f32, theta: f32, scale_r2: f32, mass: f32, rotation: f32) -> Vec2
{
    let mut rng = rand::thread_rng();
    let r = r + crate::MIN_R;
    let v = (crate::GRAVITY * mass / (r * r + scale_r2).sqrt()).sqrt();
    Vec2::new(
        rotation*v*theta.sin() + v * rng.gen_range(-crate::VEL_VARIATION..crate::VEL_VARIATION),
        rotation*-v*theta.cos() + v * rng.gen_range(-crate::VEL_VARIATION..crate::VEL_VARIATION)
    )
}
...
fn spawn_particle(
    ...
    commands.spawn((
        ...
        crate::Pose {r: pos, v: vel, prev_accel: None},
        crate::Mass(mass)
    ));
}